...
<div id="successAlert" style="display:none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); z-index: 1000;">
    ...
    $('form').on('submit', function (event) {
        ...
        $.ajax({
            ...
            data: $(this).serialize(),  // Serializes the form's elements to a query string
            dataType: "json", // Expect a json response from the server
            success: function (data) {
                var errorMessageElement = $("#alertMessage");
                ...
                if (data.success) {
                    ...
                    alertMessageElement.text(errorMessage).addClass("alert-danger").removeClass("alert-success");

                    // If the error is about booking time
                    if (errorMessage.includes("Booking already exists for this date and time.")) {
                        $('#alertTitle').text('Booking Time Alert');
